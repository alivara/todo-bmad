...
With Ember, you don't. It's a calm daily list — warm, fast, and quiet, with no streaks, badges, or gamification nagging at you. Add a task and it's there instantly; check it off and it settles with a satisfying little motion. Ember is focused today and will grow deliberately, but it will always stay calm — that's the promise, not the feature list. The thing that actually makes Ember different runs underneath: it's fully open-source and self-hostable, so you can own your data outright — or, if you'd rather not run a server, let our hosted version do it for you under the same promise of no mining and no lock-in. And ownership here is something you can exercise, not just a slogan: every task exports to an open format in one click, and you can move a hosted account onto your own server whenever you want. If we ever let you down, you leave with everything.
...
Ember is free and open-source under AGPL. Self-host it today: `git clone`, `docker compose up`, done — the full stack comes up in one command with no manual setup. Or join the hosted waitlist at [ember link] to have us run it for you when the managed version opens. Your data is always exportable to an open format in one click, and hosted accounts can migrate to your own server at any time — ownership is a feature we test, not a promise we make. Prefer to bring your team? A shared, self-hostable team workspace is on the roadmap — same promise, more people. Star the repo, file an issue, or send a pull request; Ember is built in the open and meant to stay maintained — with a real plan to keep the lights on.
...
A: Straight answer: while it's hosted by us, it sits on our servers — same as any cloud app. The difference is three specific things Todoist can't offer. One, you can export every task to an open format in one click, any time. Two, you can move a hosted account onto your own server and cut us out entirely — the code is the same open code. Three, we never mine your task text or feed it to an AI model, and because Ember is open-source, that's auditable, not just a privacy-policy sentence. So "own" for a hosted user means *you can always leave with everything and there's a real place to go.* If you want literal, physical ownership from day one, self-host — that door is open too. **[accepted trade-off — stated honestly, not hidden]**
...
### 🔨 Forged in steel
- **The abandonment-insurance answer.** AGPL + one-click export + a real business model = a genuinely convincing response to this buyer's #1 fear. Turns the Focalboard graveyard into your advantage. This is the sharpest thing in the document.
- **The honest "own" claim.** It survived because it became testable (export + hosted→self-host migration), not a slogan. Defensible under attack.
