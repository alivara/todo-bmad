...
/**
 * 1.4-E2E-001 — core capture loop (Story 1.2 create + Story 1.3 view), P0.
 * The happy path the product exists for: type a task, it appears instantly at the
 * top, the input stays ready, and it survives a reload. Network-first (no hard waits);
 * setup goes through the api via fixtures, never the UI.
 */
test.describe('@e2e @p0 create + view core loop', () => {
  test('1.4-E2E-001 add a task -> appears optimistically at top, input clears and keeps focus', async ({ page }) => {
    await page.goto('/');
  ...
  test('1.4-E2E-002 empty/whitespace title is rejected inline, no row created, no request sent', async ({ page }) => {
    let postSeen = false;
  ...
  test('1.4-E2E-004 persistence — a created task survives reload and loads automatically', async ({ page }) => {
    await page.goto('/');
    const input = page.getByPlaceholder('Add a task…');
  ...
  test('1.4-INT-001 GET /todos returns the fixed wire shape after a seeded create', async ({ page, seedTodos, request }) => {
